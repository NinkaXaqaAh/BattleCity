...
namespace RenderEngine {
	...
	void AnimatedSprite::insertState(std::string state, std::vector<std::pair<std::string, uint64_t>> subTexturesDurations) {

		m_statesMap.emplace(std::move(state), std::move(subTexturesDurations));	
	}
	...
	void AnimatedSprite::setState(const std::string& newState) {
		...
		if (it == m_statesMap.end()) {
			std::cout << "Can't find animation state" << std::endl;
			return;
		}
