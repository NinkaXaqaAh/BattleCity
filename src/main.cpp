...
int main(int argc, char** argv) {
...

    {
        ...
        while (!glfwWindowShouldClose(pWindow)) {
            ...
            uint64_t duration = std::chrono::duration_cast<std::chrono::nanoseconds>(currentTime - lastTime).count(); // times delta
            lastTime = currentTime;
			g_game.update(duration); // обновление спрайтов в игре
