...
std::shared_ptr<RenderEngine::Sprite> ResourceManager::loadSprite(const std::string& spriteName, const std::string& textureName, const std::string& shaderName, const unsigned int spriteWidth, const unsigned int spriteHeight, const std::string& subTextureName) {
	...
	if (!pShader) {
		std::cerr << "Can't find the texture: " << shaderName << "for the sprite: " << spriteName << std::endl;
		return nullptr;
	}
...
std::shared_ptr<RenderEngine::Texture2D> ResourceManager::loatTextureAtlas(std::string textureName, std::string texturePath, std::vector<std::string> subTextures, const unsigned int subTextureWidth, const unsigned int subTextureHeight) {
	auto pTexture = loadTexture(std::move(textureName), std::move(texturePath));
	if (pTexture) {
		const unsigned int texturesWidth = pTexture->width();
		const unsigned int texturesHeight = pTexture->height();
		unsigned int currentTextureOffsetX = 0;
		unsigned int currentTextureOffsetY = texturesHeight;

		for (auto& currentSubTextureName : subTextures) {

			glm::vec2 leftBottomUV(static_cast<float>(currentTextureOffsetX) / texturesWidth, static_cast<float>(currentTextureOffsetY - subTextureHeight) / texturesHeight);
			glm::vec2 rightTomUV(static_cast<float>(currentTextureOffsetX + subTextureWidth) / texturesWidth, static_cast<float>(currentTextureOffsetY) / texturesHeight);

			pTexture->addSubTexture(std::move(currentSubTextureName), leftBottomUV, rightTomUV);
			currentTextureOffsetX += subTextureWidth;
			if (currentTextureOffsetX >= texturesWidth) {
